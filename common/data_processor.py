...
class DataProcessor:
    ...
    def extract_entities(self, text: str) -> Set[str]:
        """Extract named entities from a given text using spaCy NER."""
        doc = self.ner_model(text or "")
        return {ent.text.strip() for ent in doc.ents if ent.label_}

    def enrich_nodes_with_entities(self):
        """Attach extracted entities to file nodes and update them in Neo4j if not already present."""
    ...
    def filter_entities_by_frequency(self, min_freq=2, max_percentile=0.9):
        """Filter entities by frequency thresholds to retain useful, non-generic ones."""
    ...
    def create_doc_links_by_filtered_similarity(self, top_k: int = 5, min_sim: float = 0.2):
        """Create RELATED_TO links between document pairs based on entity overlap similarity."""
    ...
    def run(self):
        """Run the full pipeline: NER enrichment → frequency filtering → semantic linking."""
